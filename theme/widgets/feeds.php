...
class BTP_Feeds_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$btp_shortcode = '[feeds ';
			$btp_shortcode .= 'include="'.$instance['include'].'" ';
			$btp_shortcode .= 'exclude="'.$instance['exclude'].'" ';	
			...
			$btp_shortcode .= 'linking="'.$instance['linking'].'" ';
		$btp_shortcode .= ']';	
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['linking'] 			= strip_tags( $new_instance['linking'] );
	...
	function form( $instance ) {
		...
		$linking_methods = array(
			'default' 		=> __('Default', 'btp_theme'),
			'new-window'	=> __('Open in new window', 'btp_theme')
		);
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'hide_icon' ); ?>"><?php _e('Hide icon', 'btp_theme'); ?></label>
			<br />
			...
			<label for="<?php echo $this->get_field_id( 'hide_label' ); ?>"><?php _e('Hide label', 'btp_theme'); ?></label>
			<br />
			...
			<label for="<?php echo $this->get_field_id( 'hide_caption' ); ?>"><?php _e('Hide caption', 'btp_theme'); ?></label>
			<br />
			
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'linking' ); ?>"><?php _e('Linking', 'btp_theme'); ?>:</label>			
			<select id="<?php echo $this->get_field_id( 'linking' ); ?>" name="<?php echo $this->get_field_name( 'linking' ); ?>" style="width:100%;">
				<?php foreach($linking_methods as $key => $value): ?>
					<?php if($key == $instance['linking']): ?>
						<option selected="selected" value="<?php echo $key; ?>"><?php echo $value; ?></option>
					<?php else: ?>
						<option value="<?php echo $key; ?>"><?php echo $value; ?></option>
					<?php endif; ?>
