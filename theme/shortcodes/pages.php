<?php
	function btp_related_pages_shortcode_generator_item() {
		return new BTP_Shortcode_Generator_Item( 
			'related_pages',
			array(
				'label'			=> '[related_pages] shortcode',
				'attributes'	=> array(
					...
					new BTP_Form_Unit_Select( 'template', array(
						'label' => 'template *',
						'choices'	=> 'btp_get_page_collection_templates'
					)),
	...
	function btp_related_pages_shortcode( $atts, $content = null ) {
		$default_template = key( btp_get_page_collection_templates() );
		...
		extract( shortcode_atts( array(
			...
			'hide'				=> '',
			'lightbox_group'	=> 'related_pages'										
			), $atts ) );
		
		$out = '';
				
		$related_ids = btp_get_related_ids($id, 'page', $max);
		if(count($related_ids)){
			...
			if($btp_query->have_posts()) {					
				...
				ob_start();
				get_template_part('/theme/parts/pages', $template);					
				$out .= ob_get_clean();
	...
	function btp_custom_pages_shortcode_generator_item() {
		return new BTP_Shortcode_Generator_Item( 
			'custom_pages',
			array(
				'label'			=> '[custom_pages] shortcode',
				'attributes'	=> array(
					new BTP_Form_Unit_Input_Text( 'ids', array(
						'label' => 'ids *',
						'hint' => __( 'Comma separated list of page ids', 'btp_theme' ),
					)),
					new BTP_Form_Unit_Select( 'template', array(
						'label' => 'template *',
						'choices'	=> 'btp_get_page_collection_templates'
