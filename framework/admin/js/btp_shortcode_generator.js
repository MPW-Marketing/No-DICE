function btpShortcodeGenerator( ed, id ) {	
	this.ed = ed;
	this.id = id;
	...
	this.showUI = function(){
		...
		tb_show( 'Shortcode Generator', '#TB_inline?width=' + windowWidth + '&height=' + windowHeight + '&inlineId=' + obj.id, null );
	...
	/**
	 * Extract name and value from input text form unit 
	 */
	this.extractInputText = function( x ) {
		var out;
	...
	/**
	 * Extract name and value from select form unit 
	 */
	this.extractSelect = function( x ) {
		var out;
	 ...
	 */
	this.extractTextarea = function( x ) {
		var out;
	...
	obj.context.find('.btp-actions .button-secondary').click(function(event){
		...
		obj.context.find('div.btp-nav select option', this).each(function(optionIndex){			
			if ( ( (jQuery(this).attr('selected') === true) || (jQuery(this).attr('selected') === 'selected') ) && optionIndex ) {
				...
				obj.context.find('.btp-viewport .btp-viewport-item').each(function(itemIndex){
					if ( ( optionIndex - 1 ) === itemIndex ) {
						...
						if ( !result.length ) {
							...
							jQuery('.btp-shortcode .btp-shortcode-attributes .btp-form-unit', this).each(function(){
								var attrModel = jQuery(this).attr( 'class' ).split(' ')[1];
								attrModel = attrModel.replace('btp-form-unit-', '');			
							...
							jQuery('.btp-shortcode .btp-shortcode-content .btp-form-unit', this).each(function(){
								var attrModel = jQuery(this).attr( 'class' ).split(' ')[1];
								attrModel = attrModel.replace('btp-form-unit-', '');			
